...
const ResultView: FC<ResultViewProps> = ({ data, onReset, onBack }) => {
   return (
      <div className="h-full w-full bg-slate-50 flex flex-col overflow-hidden animate-fade-in min-w-0">
         {/* Page Header */}
         <div className="flex-none bg-white px-4 py-4 sm:px-6 md:px-12 md:py-8 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-3 sm:gap-6 min-w-0">
               <button onClick={onBack} className="p-2.5 sm:p-3 bg-slate-50 hover:bg-slate-100 rounded-2xl text-slate-400 transition-colors flex-shrink-0">
                  <ArrowLeft size={20} />
               </button>
               <div className="min-w-0">
                  <h2 className="text-xl sm:text-2xl md:text-3xl font-black text-slate-900 tracking-tight truncate sm:whitespace-normal">Active Protocol</h2>
                  <p className="text-slate-500 font-medium text-sm sm:text-base">Custom engineered for your biological profile.</p>
               </div>
            </div>
            <div className="flex items-center gap-3 flex-shrink-0">
               <button onClick={onReset} className="flex items-center gap-2 px-4 py-2.5 sm:px-6 sm:py-3 bg-indigo-50 text-indigo-600 rounded-2xl font-bold text-sm hover:bg-indigo-100 transition-all">
         ...
         <div className="flex-1 overflow-y-auto no-scrollbar p-4 sm:p-6 md:p-8 lg:p-12 grid grid-cols-1 xl:grid-cols-12 gap-6 md:gap-8 lg:gap-12">

            {/* Column 1: Summary & Forecast (xl:col-span-3) */}
            <div className="xl:col-span-3 space-y-6 md:space-y-8">
               <section className="bg-slate-900 rounded-2xl md:rounded-[2.5rem] p-5 md:p-8 text-white relative overflow-hidden shadow-2xl">
               ...
               <section className="space-y-4">
                  <h3 className="text-xs sm:text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2 px-2">
                     <TrendingUp size={16} className="text-indigo-600" /> Forecasted Milestones
                  </h3>
                  <div className="space-y-3 md:space-y-4">
                     {data.milestones.map((m, i) => (
                        <div key={i} className="bg-white p-4 md:p-6 rounded-xl md:rounded-[2rem] border border-slate-100 shadow-sm">
            ...
            <div className="xl:col-span-4 space-y-6 md:space-y-8">
               <section className="bg-white rounded-2xl md:rounded-[2.5rem] p-5 md:p-8 lg:p-10 border border-slate-100 shadow-sm space-y-5 md:space-y-8">
            ...
            <div className="xl:col-span-5 space-y-4 md:space-y-8">
               <section className="space-y-4 md:space-y-6">
                  <h3 className="text-lg sm:text-2xl font-black text-slate-900 flex items-center gap-3">
                     <Dumbbell size={24} className="text-indigo-600 flex-shrink-0" /> Training Periodization
                  </h3>
                  <div className="space-y-4 md:space-y-6">
                     {data.weeklySchedule.map((day, i) => (
                        <div key={i} className="bg-white rounded-2xl md:rounded-[2.5rem] p-5 md:p-8 border border-slate-100 shadow-sm transition-all hover:shadow-md">
                           <div className="flex justify-between items-center mb-4 md:mb-6">
                              <div className="min-w-0">
                                 <span className="text-lg sm:text-xl font-black text-slate-900 block truncate">{day.day}</span>
                                 <div className="text-xs font-bold text-indigo-600 mt-1 uppercase tracking-widest">{day.focus}</div>
                              </div>
                              <Zap size={24} className="text-indigo-200 flex-shrink-0" />
                           </div>
