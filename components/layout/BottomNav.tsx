    ...
    >
      {BOTTOM_NAV_LINKS.map(({ id, label }) => (
        <button
          ...
          className={`flex flex-col items-center justify-center gap-0.5 min-w-0 flex-1 py-2 px-1 rounded-xl transition-colors ${
            activeTab === id ? 'text-indigo-600 bg-indigo-50' : 'text-slate-400 hover:text-slate-600'
          }`}
          aria-label={label}
        >
          {ICONS[id]}
