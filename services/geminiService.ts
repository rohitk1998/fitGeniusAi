...
// Lazy-init client so the app loads even if API key is missing or SDK throws at init.
let _ai: InstanceType<typeof GoogleGenAI> | null = null;
function getAI(): InstanceType<typeof GoogleGenAI> {
  if (_ai) return _ai;
  // Vite injects process.env.GEMINI_API_KEY / process.env.API_KEY at build time
  const apiKey = import.meta.env.VITE_API_KEY ?? "";
  console.log(apiKey, "sdad");
